        ...
        {ACTIONS.filter(a => (grant.actions & a.bit) === a.bit).map(a => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex items-center gap-1 rounded-md border px-2 py-1 text-xs transition disabled:cursor-not-allowed disabled:opacity-40',
                active ? 'border-highlight bg-highlight/10 text-highlight' : 'border-border text-muted-foreground hover:text-foreground',
              )}
            >
              {a.label()}
            </button>
