...
import { useClanMembers, useClanRoles, useCreateRole, useDeleteRole, useGrantCatalog, useReorderRoles, useSetRoleGrants, useUpdateRole } from './clan.hooks'
...
export function RoleManagerModal({ clanId, isOwner, open, onClose }: Props) {
  ...
  useEffect(() => { setOrder(sortableIds) }, [sortableIds.join(',')])
  ...
  const canManage = (role: ClanRoleDetail) => role.key !== 'owner' && ceiling > role.position

  const onCreate = () =>
    create.mutate(
      { name: m.clan_role_new_name() },
      { onSuccess: (role) => { setSelectedId(role.id); toast.success(m.clan_role_created()) }, onError: (e) => toast.error(resolveClanError(e)) },
    )
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    setOrder(next)
    reorder.mutate(next, { onSuccess: () => toast.success(m.clan_roles_reordered()), onError: (err) => { toast.error(resolveClanError(err)); setOrder(sortableIds) } })
  }
      ...
      <div className="flex h-[60vh]">
        <aside className="flex w-60 shrink-0 flex-col border-r border-border">
          <div className="flex items-center justify-between border-b border-border px-3 py-3">
            ...
            <button type="button" onClick={onCreate} aria-label={m.clan_role_new()} className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground">
              <FontAwesomeIcon icon={faPlus} className="text-xs" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {ownerRole && <RoleRow role={ownerRole} selected={false} draggable={false} selectable={false} onSelect={() => {}} />}
        ...
        <section className="flex-1 overflow-y-auto p-5">
          {selected && catalog ? (
            <RoleEditor key={selected.id} clanId={clanId} role={selected} catalog={catalog} effective={effective} editable={canManage(selected)} onDeleted={() => setSelectedId(null)} />
          ) : (
...
function RoleRow({ role, selected, draggable, selectable, onSelect }: { role: ClanRoleDetail; selected: boolean; draggable: boolean; selectable: boolean; onSelect: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn('flex items-center gap-1.5 rounded-lg px-1.5 py-2', selected ? 'bg-accent' : 'hover:bg-muted', isDragging && 'opacity-60')}>
      {draggable ? (
      ...
      <button type="button" disabled={!selectable} onClick={() => onSelect(role.id)} className="flex min-w-0 flex-1 cursor-pointer items-center gap-2 text-left disabled:cursor-default">
...
function RoleEditor({ clanId, role, catalog, effective, editable, onDeleted }: { clanId: string; role: ClanRoleDetail; catalog: GrantCatalogEntry[]; effective: Record<string, number>; editable: boolean; onDeleted: () => void }) {
  ...
  const nameChanged = !role.isSystem && name.trim() !== role.name
  const colorChanged = color !== role.color
  const grantsChanged = !sameGrants(grants, role.grants)
  const dirty = nameChanged || colorChanged || grantsChanged
  const saving = update.isPending || setRoleGrants.isPending

  const save = async () => {
    try {
      if (nameChanged || colorChanged) await update.mutateAsync({ roleId: role.id, payload: { ...(nameChanged ? { name: name.trim() } : {}), ...(colorChanged ? { color } : {}) } })
  ...
  const onDelete = () => del.mutate(role.id, { onSuccess: () => { toast.success(m.clan_role_deleted()); onDeleted() }, onError: (e) => toast.error(resolveClanError(e)) })

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        <TextField label={m.clan_role_name()} value={name} disabled={!editable || role.isSystem} maxLength={40} onChange={(e) => setName(e.target.value)} />
      ...
      <div className="flex flex-col gap-3">
        ...
        <GrantEditor catalog={catalog} value={grants} effective={effective} disabled={!editable} onToggle={(grant, actions) => setGrants((prev) => ({ ...prev, [grant]: actions }))} />
      </div>
      ...
      <div className="flex items-center justify-between gap-2 border-t border-border pt-4">
        ...
        <Button size="sm" onClick={save} loading={saving} disabled={!editable || !dirty}>{m.clan_role_save()}</Button>
      </div>
    </div>
  )
}
...
function toRows(g: Record<string, number>): ClanRoleGrantView[] {
  return Object.entries(g).filter(([, a]) => a > 0).map(([grant, actions]) => ({ grant, actions }))
}
